...
var SRBLib = (function () {
  ...
  var pub = {
    ...
    /**
     * This method should be called in the controllers onInit function, it creates a SrbSignaturePad object and adds it to the global array
     * SRBLib.srbSignaturePads. It then adds the canvas element to the given parentElement
     * TODO should we add a style parameter, eg. for the dotted borde-r?
     * @public
     * @memberOf module:SRBLib
     * @author Georg Koschitz - SRB Consulting Team
     * @param {Object} parentElement - the parent element object in the view.xml (should look like this. <core:HTML id="html"></core:HTML>)
     * @param {String} [saveName=savedImage] - will probably not be needed, since the data is atm saved to the object in the global array SRBLib.srbSignaturePads
     * @param {Number} [sizeWidth=40%] - the width of the canvas element in percent
     * @param {Number} [sizeHeight=20%] - the height of the canvas element in percent
     * @param {String} [bgColor=rgb(255, 255, 255)] - the background color of the canvas element, rgb notation needs to be used
     * @param {String} [saveEncoding=base64]	'base64' | '?'
     * @param {String} [saveFileType=jpg]		'jpeg' | 'jpg' | 'png' | 'svg'
     * @returns {Object} SrbSignaturePad object containing all relevant data
     */
    addSignaturePad: function (parentElement, saveName, sizeWidth, sizeHeight, bgColor, saveEncoding, saveFileType) {
      // check parentElement
      try {
        if (parentElement === undefined) {
          throw "parentElement object undefined!";
        } else if (!typeof (parentElement === "object")) {
          throw "parentElement not an object!";
        }
      } catch (e) {
        console.log(e);
        return e;
      ...
      // check sizeWidth
      var sWidth = parseFloat(sizeWidth);
      if (sizeWidth === undefined || isNaN(sWidth) || sWidth < 0 || sWidth > 100) {
        //sizeWidth = $(window).width()*0.4;
        sizeWidth = "40%";
      }
      // check sizeHeight
      var sHeight = parseFloat(sizeHeight);
      if (sizeHeight === undefined || isNaN(sHeight) || sHeight < 0 || sHeight > 100) {
        //sizeHeight = $(window).height()*0.2;
        sizeHeight = "20%";
      }
      // check bgColor
      if (bgColor === undefined || !this.isValidRgb(bgColor)) {
        bgColor = "rgb(255, 255, 255)";
      }
      // check saveEncoding
      if (saveEncoding === undefined || !this.isValidImageEncoding(saveEncoding)) {
        saveEncoding = "base64";
      }
      // check saveFileType
      if (saveFileType === undefined || !this.isValidImageFileType(saveFileType)) {
        saveFileType = "jpg";
      }
      // create new SrbSignaturePad
      var SrbSignaturePad = {
        ...
        idPad: parentElement.sId + "_signature-pad",
        encodedImageData: ""
      };
      // if the global array is not there, create it
      if (typeof SRBLib.srbSignaturePads === "undefined") {
        SRBLib.srbSignaturePads = new Array();
      }
      // add the SrbSignaturePad to the global array
      SRBLib.srbSignaturePads.push(SrbSignaturePad);
      // finally add the needed html to the page
      SrbSignaturePad.parentElement.setContent(
        "<div id='" +
        SrbSignaturePad.idPad +
        "' class='signature-pad'><canvas width='" +
        SrbSignaturePad.sizeWidth +
        "' height='" +
        SrbSignaturePad.sizeHeight +
        "' style='border:dotted'></canvas></div>"
      );
      return SrbSignaturePad;
      //return SRBLib.srbSignaturePads.push(SrbSignaturePad)-1; // would be return of index in global array
    },
